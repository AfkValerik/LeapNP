...
class RelationMessagePassing(nn.Module):
    def __init__(self, relations: List[Tuple[int, int]], hidden_size: int):
        ...
        self.update_num = nn.Sequential(nn.Linear(2 * self.num_inputs, 2 * self.num_inputs, True), nn.ReLU(), nn.Linear(2 * self.num_inputs, self.num_inputs, True))
    ...
    def forward(self, node_states: Tuple[Tensor,Tensor], relations: Dict[int, Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        # Update states with aggregated messages
        next_node_states = self.update(torch.cat([max_msg,node_states[0]], dim=1))
        num_next_node_states = self.update_num(torch.cat([sum_msg,node_states[1]], dim=1))
        next_node_states = [next_node_states,num_next_node_states]
...
class AddMaxNumericModelValuesReadout(pl.LightningModule):
