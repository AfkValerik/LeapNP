...
from data_structures import encode_state,encode_goal_state, extract_condition_object_pairs, get_encoded_states_with_values, extract_boolean_goals_pairs
from architecture import g_model_classes

class GnnBooleanPolicy(NNHeuristic):
    def __init__(self, model, predicates, goal_predicates, objects, obj_encoding, num_conditions,augmented_goals,encoding,goals,goal_encoding,constants):
        self.model = model
    ...
    def __encode_gnn_state__(self,state):
        ...
        for key,value in self.num_conditions.items():
            encoded_state = extract_boolean_goals_pairs(state,key,value,encoded_state)
...
def gnnHeuristic(path,predicates,goal_predicates,objects,obj_encoding,num_conditions,augmented_goals_map,goals,aggregation,readout,gpus,constants):
    ...
    return GnnBooleanPolicy(model, predicates, goal_predicates, objects, obj_encoding, num_conditions,augmented_goals_map,model.encoding,goals,goal_encoding,constants)
