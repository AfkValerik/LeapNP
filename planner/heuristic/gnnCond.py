...
from data_structures import encode_gnn_state, encode_goal_state, extract_numeric_goals, extract_conditions, encode_state, extract_numeric_goals_in_state
from architecture import g_model_classes

class GnnCond(NNHeuristic):
...
def gnnHeuristic(path,predicates,goal_predicates,objects,obj_encoding,num_conditions,goal_num_conditions,num_conditions_in_goal,goals,aggregation,readout,gpus,state,constants):
    ...
    return GnnCond(model, predicates, goal_predicates, objects, obj_encoding, num_conditions, goal_num_conditions,num_conditions_in_goal,model.encoding,goals,goal_encoding,constants)
