...
def analyze_problem(domain, instance,grounder):
    ...
    ground_map = getattr(groundedProblem, 'map_back_action_instance', None)
    if ground_map is not None and hasattr(ground_map, 'keywords') and 'map' in ground_map.keywords:
        ground_map_dict = ground_map.keywords['map']
    else:
        ground_map_dict = None
    groundedActions = standardizeActions(groundedActions, objects, ground_map=ground_map_dict)
