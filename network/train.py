...
from architecture import set_suboptimal_factor, set_loss_constants, MaxModelBase
...
from architecture.max_base import MaxModelBase
...
def _parse_arguments():
    ...
    default_max_samples_per_file = 10000
    ...
    default_n_actions = 'True'
    default_numeric = True
    default_num_inputs = True
    default_architecture = "conditions"
    default_actions_readout = False
    
    # required arguments or --resume that requires a path
    parser.add_argument('--train', required=True, type=str, help='path to training dataset')
    parser.add_argument('--validation', required=True, type=str, help='path to validation dataset')
    ...
    parser.add_argument('--domain_name',required = True, type = str, help = 'name of the domain')
    # arguments with meaningful default values
    parser.add_argument('--architecture', default=default_architecture, nargs='?', choices=['conditions', 'fluents', "fluents_conditions"], help=f'architecture to use (default={default_architecture})')
    parser.add_argument('--actions_readout', default=default_actions_readout, type = bool, help = 'whether to give a different output value for each action (default=True)')
    parser.add_argument('--num_inputs', default = default_num_inputs, type = bool, help = 'whether numeric values are used as input in the GNN')
    parser.add_argument('--numeric',default = default_numeric, type = bool, help = 'whether the dataset includes numeric values or not')
    parser.add_argument('--n_actions', default=default_n_actions, type=bool, help=f'actions included in the training set (default={default_n_actions})')
    parser.add_argument('--aggregation', default=default_aggregation, nargs='?', choices=['add', 'max', 'smoothmax', 'mean'], help=f'readout aggregation function (default={default_aggregation})')
    ...
    parser.add_argument('--max_epochs',default = default_max_epochs, type = int, help = 'maximum number of epochs to train the model')
...
#    if args.num_inputs is None or args.num_inputs == "False": args.num_inputs = False
...
#TODO: modify all the code below in order to adapt it to the new dataset format

def _load_datasets(args):
    print(colored('Loading datasets...', 'green', attrs = [ 'bold' ]))
    try:
        load_dataset, collate = g_dataset_methods[(args.loss,args.numeric,args.num_inputs,args.architecture,args.actions_readout)]
    except KeyError:
        raise NotImplementedError(f"Loss function '{args.loss}'")
    (train_dataset, predicates) = load_dataset(args.train, args.max_samples_per_file, args.max_samples, args.verify_datasets,args.domain_name)
    (validation_dataset, _) = load_dataset(args.validation, args.max_samples_per_file, args.max_samples, args.verify_datasets,args.domain_name)
    loader_params = {
    ...
    if args.actions_readout:
        toOut = predicates[0]
        print(f'{len(toOut)} predicate(s) in dataset; predicates=[ {", ".join([ f"{name}/{arity}/{variables}" for name, arity,variables in toOut ])} ]')
    elif args.num_inputs:
        print(f'{len(predicates)} predicate(s) in dataset; predicates=[ {", ".join([ f"{name}/{arity}/{variables}" for name, arity,variables in predicates ])} ]')
    else:
...
def _load_model(args, predicates):
    ...
    model_params = {
        ...
        "actions_readout": args.actions_readout,
        "numeric": args.numeric,
        "num_inputs": args.num_inputs,
    }

    try:
        Model = g_model_classes[(args.aggregation, args.readout, args.loss,args.numeric,args.num_inputs, args.actions_readout)]
    except KeyError:
...
def _load_trainer(args):
    ...
    callbacks.append(ModelCheckpoint(save_top_k=args.save_top_k, monitor='validation_loss', filename='{epoch}-{step}-{validation_loss}-{train_loss}'))
    ...
    if args.gpus > 0: trainer = pl.Trainer(accelerator= "gpu", devices=args.gpus , **trainer_params)
