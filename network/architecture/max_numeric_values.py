...
class RelationMessagePassing(nn.Module):
    ...
    def forward(self, node_states: Tensor, relations: Dict[int,Tensor], num_inputs : Dict[int,Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        max_msg = torch.full_like(node_states, -float('inf'), device=self.get_device())
        outputs = []
        ...
        for output, node_indices in outputs:
            scattered_output = torch.scatter(max_msg, 0, node_indices, output)
            max_msg,_ = torch.max(torch.stack([max_msg, scattered_output]), dim=0)
...
class MaxNumericModelValues(pl.LightningModule):
