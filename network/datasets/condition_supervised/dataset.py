...
from datasets import load_directory, load_file_spanner, get_condition_encoded_states, get_condition_relations
...
def load_directory(path: str, max_samples_per_file: int, max_samples: int, filtering_fn=None, load_file_fn=None, verify_states: bool = False,domain_name : str = None):
    ...
    with open(path + domain_name + ".json", 'r') as file:
        data = json.load(file)
    predicates,cond_list = get_condition_relations(data)
    ...
    for file in os.listdir(path):
        if '.json' in file:
            if file != domain_name + ".json":
                ...
                labeled_states.extend(get_condition_encoded_states(instance,cond_list,max_samples_per_file= max_samples_per_file))
